...
from articles.models import Book, Section

def update_book_45(docx_path):
    if not os.path.exists(docx_path):
        print(f"שגיאה: קובץ הוורד {docx_path} לא נמצא. אנא ודא שהעלית אותו לתיקייה.")
        return
        
    print("קורא את קובץ הוורד (ושואב ממנו את הערות השוליים)...")
    ...
    # 1. חילוץ כל הערות השוליים מהוורד למילון
    footnotes_dict = {}
    ...
    # 2. שימוש מדויק בלוגיקה של הסקריפט הישן שלך כדי לפרק את הוורד לאותם סעיפים!
    ...
    for el in soup.find_all(['h1', 'h2', 'h3', 'p', 'ul', 'ol', 'table']):
        ...
        if text.startswith("סימן ") or text in ["פתח דבר", "הקדמה", "קונטרס ראשון גדרי שליחות", "קונטרס שני גדר בפני נכתב ובפני נחתם", "דינים העולים מהספר"]:
            if current_section_html:
                html_sections.append(current_section_html)
                current_section_html = []
            is_in_intro = True
            continue
    ...
    # 3. שליפת הסעיפים מתוך מסד הנתונים (שומר על ה-ID המקורי!)
    db_sections = list(Section.objects.filter(chapter__book_id=45).order_by('chapter__order', 'order'))
    
    print(f"נמצאו {len(html_sections)} סעיפים בקובץ הוורד.")
    ...
    # 4. מיזוג התוכן מהוורד לתוך המסד הקיים!
    for i, db_sec in enumerate(db_sections):
        if i < len(html_sections):
            html_content = force_large_css + "".join(html_sections[i])
            
            # בדיקה אילו הערות שוליים שייכות לסעיף הנוכחי
            sec_soup = BeautifulSoup(html_content, 'html.parser')
            used_fns = {}
            ...
            # אם יש הערות לסעיף, נוסיף אותן בתחתית בצורה מעוצבת
            if used_fns:
                ...
                for fn_id, li in used_fns.items():
                    fn_list += str(li)
                fn_list += "</ol>"
                html_content += separator + title + fn_list
            
            # פעולת השמירה - לא יוצרת סעיף חדש, אלא רק מעדכנת את הקיים!
            db_sec.content = html_content
            db_sec.save()
            print(f"עודכן בהצלחה סעיף: {db_sec.title} (ID מקורי נשמר: {db_sec.id})")

    print("\nהפעולה הסתיימה! כל הקישורים של הספר ושל כל סעיף נשמרו בדיוק כפי שהיו.")

if __name__ == '__main__':
    update_book_45('gidrei.docx')